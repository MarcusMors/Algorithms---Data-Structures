...
template<typename T, bool sorted = false> struct AVL_tree
{
  using value_type = T;
  // class iterator;
  ...
  AVL_tree() = default;
  // cppcheck-suppress noExplicitConstructor ; allowing AVL_tree A = {1,2,3} and A{1,2,3}
  // AVL_tree(const sort_list<T> list);
  // AVL_tree(const forward_sort_list<T> list);
  // int count{ 0 };
  // int level_count{ 1 };
  ...
  //   ++count;
  // }
  AVL_tree(const std::initializer_list<T> init)
  {
    if (sorted) {
      ...
      root->left = m_fill_branch(begin, begin + mid);
  ...
  bin_search_node<T> *m_fill_branch(const T *t_begin, const T *t_end)
  {
    ...
    const T mid = *(t_begin + (t_end - t_begin / 2));
  ...
  AVL_tree(const AVL_tree &tree) : root{ tree.root } {}


  ~AVL_tree() { m_delete_branches(root); }
  ...
  bool find(const T &t_value, bin_search_node<T> **&t_ptr, int &t_height);
...
template<class T> ostream &operator<<(std::ostream &os, const AVL_tree<T> &t_tree)
{
  // print left
  // os << t_tree
  // print right
...
bool fstd::AVL_tree<T, sorted>::find(const T &t_value, bin_search_node<T> **&t_ptr, int &t_height)
{
  t_ptr = &root;
...
template<class T, bool sorted> bool fstd::AVL_tree<T, sorted>::insert(const T t_value)
{
  bin_search_node<T> **ptr;
  int height = 0;
  if (find(t_value, ptr, height)) { return false; }
  // balanced?
  // Balance factor (BF)
  // BF(node) = H(node.right) - H(node.left); // [-1,0,1]
  //
  return true;
}
// emplace front
template<class T, bool sorted>
fstd::bin_search_node<T> **fstd::AVL_tree<T, sorted>::reposition(bin_search_node<T> **&t_ptr)
{
...
template<class T, bool sorted> bool fstd::AVL_tree<T, sorted>::remove(const T t_value)
{
  bin_search_node<T> **ptr;
  int height = 0;
  if (!find(t_value, ptr, height)) { return false; }
...
// template<class T> void fstd::AVL_tree<T>::swap(AVL_tree &t_list)
// {
//   AVL_tree temp{ *this };
//   *this = t_list;
//   t_list = temp;
