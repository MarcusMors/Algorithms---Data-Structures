...
template<class N, class E> void CGraph<N, E>::insertNode(N node)
{
  Node *temp = new Node;
  temp->m_data = node;
...
template<class N, class E> void CGraph<N, E>::insertEdge(E edge, Node *a, Node *b, bool dir)
{
  Edge *temp = new Edge;
  temp->m_data = edge;
...
template<class N, class E> void CGraph<N, E>::removeEdge(E edge, Node *a, Node *b)
{
...
template<class N, class E> void CGraph<N, E>::removeNode(Node *a)
{
...
template<class N, class E> void CGraph<N, E>::print()
{
  for (int i = 0; i < m_nodes.size(); i++) {
    ...
    for (typename std::list<Edge *>::iterator it = m_nodes[i]->m_edges.begin(); it != m_nodes[i]->m_edges.end(); ++it) {
      if ((*it)->m_nodes[0] == m_nodes[i])
        ...
        std::cout << "V" << (*it)->m_nodes[1]->m_data << ", W" << (*it)->m_data
                  << " -> ";// cuando la arista tiene direccion 1

      else if (!(*it)->m_dir)
        // std::cout <<(*it)->m_data <<" "<< (*it)->m_nodes[0]->m_data<< " <-> "; //cuando la arista es bidireccional
        std::cout << "V" << (*it)->m_nodes[0]->m_data << ", W" << (*it)->m_data
                  << " <-> ";// cuando la arista es bidireccional
...
int main()
{
  ...
  g.insertEdge(2, g.m_nodes[0], g.m_nodes[1], false);
  // g.insertEdge(10, g.m_nodes[0], g.m_nodes[4], false);

  // g.insertEdge(3, g.m_nodes[1], g.m_nodes[2], false);
  // g.insertEdge(7, g.m_nodes[1], g.m_nodes[4], false);

  // g.insertEdge(4, g.m_nodes[2], g.m_nodes[0], false);
  // g.insertEdge(4, g.m_nodes[2], g.m_nodes[3], false);

  // g.insertEdge(5, g.m_nodes[3], g.m_nodes[4], false);

  // g.insertEdge(3, g.m_nodes[4], g.m_nodes[2], false);
