...
// class graph_type = graph<t_node_value_type, t_edge_value_type>>
  ...
  t_edge_value_type &t_cont)
{
  ...
  if (t_begin == t_end) { return t_path; }
  ...
  for (typename std::list<edge_type *>::iterator edge_list_it = (t_begin->edges).begin();
       edge_list_it != (t_begin->edges).end();
       ++edge_list_it) {

    bool repeated_node{ false };

    // for (typename std::list<node_type *>::iterator it = t_path.begin(); it != t_path.end(); it++) {
    //   if (*it == (*edge_list_it)->node[0]) { repeated_node = true; }// POSSIBLE
    // }
    ...
    auto local_path = secondary_call_dijkstra((*edge_list_it)->second_end, t_end, path_copy, cont_copy);
